...
def calculate_eigen_values():
    ...
    if matrix_size.get() == '2x2':
        try:
            ...
            result.config(text=f'Eigen Values are : \n\n {(eigen_values[0]):.2f} \n\n{(eigen_values[1]):.2f}',font='Arial 12')
...
def update_matrix_entries():
    global matrix_size,entry_2x2,entry_3x3,result
...
def eigen_values():
    ...
    root = tk.Tk()
    root.title("Matrix Eigen Value Calculator")
    ...
    # Button to calculate 
    calc_button = tk.Button(root, text="Calculate Eigen Values ", command=calculate_eigen_values,font='Calibri 13')
